...
var startRouter = function startRouter(views, rootStore) {
  ...
  history.subscribe(function (location, action) {
    ...
    if (match.route.context) {
      var _match$route$context = match.route.context,
          routeName = _match$route$context.routeName,
          params = _match$route$context.params;
      var route = store.routes[routeName];
    ...
    var curr = (0, _utils.buildLookupPath)(store.currentRoute);

    if (!runAllEvents) {
      newPath = newPath.filter(function (route, i) {
        return route.isActive && curr.includes(route) && route["final"] && i === newPath.length - 1 || !route.isActive || i === newPath.length - 1 && route === store.currentRoute;
      });
    } // build params
    ...
    if (newPath.length > 0 && oldPath.length > 0 && newPath[newPath.length - 1].slot !== oldPath[0].slot && oldPath[0].context !== false) {
      var _routeName = oldPath[0].context.routeName;
    ...
    fns.reduce(function (promiseChain, currentTask) {
      ...
      store.currentRoute = match.route; // set previous location
    ...
    .then(function () {
      for (var _i in oldPath) {
        if (oldPath[_i].context !== false) {
          oldPath[_i].context = null;
        }

        oldPath[_i].isActive = false;
      }
    });
