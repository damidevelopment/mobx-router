...
var setCurrentRoute = function setCurrentRoute(routerStore) {
  return (0, _mobx.action)(function () {
    ...
    for (var name in routerStore.routes) {
      var route = routerStore.routes[name];
      var params = (0, _utils.buildParamsObject)(route.pattern, route.defaultParams);

      if (params) {
        routerStore.params = params;
        routerStore.queryParams = (0, _utils.getQuery)();
...
var createDirectorRouter = function createDirectorRouter(views, rootStore) {
  ...
  director.configure({
    ...
    async: true,
    html5history: true,
    before: [setCurrentRoute(routerStore)].concat((0, _utils.buildFnsArray)(onEnter).map(function (fn) {
      return (0, _utils.compileSyncAction)(rootStore, fn);
    })),
    after: (0, _utils.buildFnsArray)(onExit).map(function (fn) {
      return (0, _utils.compileSyncAction)(rootStore, fn);
    }),
...
var startRouter = function startRouter(views, rootStore, _ref3) {
  var currentView = _ref3.currentView,
      config = _objectWithoutProperties(_ref3, ["currentView"]);

  var routes = buildRoutes(views);
